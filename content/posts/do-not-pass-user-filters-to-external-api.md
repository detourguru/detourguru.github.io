...
export function filterShows(shows: Show[], filters: ShowFilters): Show[] {
  ...
  const areaNames = filters.signgucode && AREA_NAMES_BY_CODE[filters.signgucode];
...
| | 필터를 그대로 넘겼을 때 | 지금 |
|---|---|---|
| 캐시 항목 수 | 최대 135조합 × 페이지 수 (검색어 포함 시 무한) | 장르 2개 × 페이지 수 |
| 사용자가 필터를 바꿀 때 | 새 조합이면 미스 -> 외부 호출 | 히트, 외부 호출 0 |
| 1시간 동안 외부 호출 | 들어온 조합 수만큼 | 캐시 채울 때 한 번 |

근데 솔직히 이건 아직 계산일 뿐이다. 실제로 배포한 뒤에 호출이 몇 번 나갔는지 응답이 얼마나 빨라졌는지는 아직 못 쟀다. 캐시 히트/미스를 남기는 로그를 안 붙여둔 상태라 지금은 확인할 방법 자체가 없다.

다음엔 fetch 앞뒤로 시각이랑 히트 여부를 찍어두고 하루치 로그로 미스율부터 보려고 한다. 최소한 "필터를 여러 번 바꿔도 외부 호출은 안 늘어난다"는 건 숫자로 확인해두는 게 맞는 것 같다. 설계상 그럴 수밖에 없다고 생각하는 것과 실제로 그런 걸 본 건 다른 얘기니까.
