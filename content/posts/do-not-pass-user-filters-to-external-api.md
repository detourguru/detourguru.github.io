...
description = 'KOPIS 공연 API에 사용자 필터를 쿼리 파라미터로 그대로 넘기면, 요청 URL을 캐시 키로 쓰는 Next.js Data Cache에서는 필터 조합마다 캐시가 따로 생긴다. 필수 조건으로 3개월치 목록만 받아 캐싱하고 장르·지역·검색어 필터는 받아온 배열에서 처리하도록 바꿨다.'
tags = ["설계", "성능 최적화"]
+++
