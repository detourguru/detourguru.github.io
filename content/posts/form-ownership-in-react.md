...
지금은 에디터의 27개 편집 폼이 전부 이 구조로 돌아간다. formConfig에는 schema와 defaultValues 외에도 transformSubmit(폼 값을 서버 계약 형태로 변환해주는 함수)까지 함께 선언해서 폼 구동 관련은 전부 Layout이 담당한다.
...
물론 트레이드 오프도 있었다. 각 엔티티에 붙은 form 마다 스키마가 다르다보니 Layout의 useForm은 특정 폼 타입으로 고정할 수 없어 타입이 느슨해질 수밖에 없었고 (이건 스키마 단위 테스트로 보완했다), 폼 인스턴스 하나를 공유하는 대가로 reset 책임이 계속 따라다녔다. 그래도 27개 폼의 보일러플레이트를 없앤 것에 비하면 감수할 만한 비용이었다.
