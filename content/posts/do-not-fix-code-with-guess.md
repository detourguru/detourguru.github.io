...
이 현상에는 사실 여러가지 원인이 있었는데 하나는 `fetch`가 4xx/5xx 응답을 reject하지 않는다는 것이고, 다른 하나는 우리 팀의 공통 통신 모듈이 어떤 실패든 `throw`하지 않고 `{ data, error }`를 그대로 `return`하도록 짜여 있다는 것이었다. 에디터 특성 상 화면이 깨지지 않게 하기위해 그렇게 설계한거였는데, 덕분에 404가 에러가 아니라 값으로 바뀌어서 React Query의 에러 경로에도 안 걸린 채 컴포넌트까지 내려왔다. (이 래퍼가 만든 사각지대는 나중에 [sentry 캡처도 역할 분리가 필요하다](/posts/sentry-blind-spot/)에서도 문제가 된다...)

그러니까 예외로 던져지지 않는 실패였기 때문에 애초에 try-catch로 잡힐 수가 없는 구조였다. 원인을 알고 나니 수정은 명확했다. 던져지지 않는다면 값으로 다뤄야 한다.
...
데이터 없음 상태를 `null`이라는 명시적인 값으로 내어주고나니 더 이상 undefined가 몰래 컴포넌트까지 새어 들어오지 않게되었다. 호출부에서도 `game === null`이면 빈 상태 UI를 그리면 되니 분기가 눈에 보이게 됐다.
