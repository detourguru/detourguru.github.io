...
description = 'React Query로 갈아탔더니 페이지 이동마다 API가 29개, 뮤테이션 한 번에 109건씩 나갔다. 래퍼 훅의 fallback 누락과 쿼리 전역 무효화를 잡은 기록.'
tags = ["React", "React Query", "캐싱", "성능 최적화"]
+++
...
이 비슷한 전역 무효화 패턴은 에디터 저장 쪽에도 숨어 있었다. 에디터에서 실시간으로 데이터가 업데이트된 것처럼 보여주기 위해 저장 성공 콜백에 `invalidateQueries()`를 추가해뒀었는데, 데이터가 꽤 많아진 후부터는 저장 한번을 할때마다 화면이 거의 멈추다시피 했다. 실측해보니 저장 1회에 fetch가 54건 나가고 refetch가 한꺼번에 몰리면서 메인스레드가 약 11초 블로킹되고 있었다.
...
에디터 저장 쪽도 엔티티별 무효화 대상을 매핑해 타깃 무효화로 교체해줬고 저장할 때마다 화면이 멈추던 증상이 사라졌다. 데이터가 커서 당연히 오래 걸리는 거라 생각하고 최적화를 뒤로 미뤘는데 정작 범인은 데이터 크기가 아니라 무효화 범위였던 것이다. 재발 방지를 위해 `저장 시 전역 invalidate 0회` 케이스를 테스트로 강제해서 누군가 다시 인자 없는 `invalidateQueries()`를 넣으면 테스트가 깨지게 해뒀다.

## 이번에 배운 점

기본적으로는 솔직히 둘 다 React Query의 기본 동작을 정확히 이해하지 못해서 생긴 문제였다.
