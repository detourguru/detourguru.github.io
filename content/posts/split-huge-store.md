...
참고로 이 기준으로 계속 나눈 결과 지금은 게임 런타임 상태만 store가 21개가 됐다. 좀 많긴 하지만 이름만 봐도 담당 범위가 보이는 크기라 오히려 하나였을 때보다 관리가 쉬워졌다.

정리하면, store를 나눌지 말지는 이런 기준으로 판단하면 될 것 같다.

- 상태들의 변경 주체나 생명주기가 서로 다르다면 -> 도메인 기준으로 쪼갠다
- 하나의 액션이 여러 도메인 상태를 같이 건드리고 있다면 -> 그 조합은 store 내부가 아니라 페이지/기능 단위 훅으로 꺼낸다

store 개수가 늘어나는 게 부담스럽다면 한 번에 다 쪼갤 필요는 없다. 실제로 자주 같이 바뀌는 상태부터 먼저 나눠보고 나머지는 남겨둬도 괜찮다는 것이 나의 생각이다.
