...
description = '색약인 동료가 배지 색을 구분하지 못해서 WCAG 대비비를 계산해보니 테마 10개 중 6개가 4.5:1 기준에 못 미쳤다. 글자색을 흰색이나 검정 하나로 고정하면 어느 쪽이든 실패해서, 강조색과 짝을 이루는 --color-accent-foreground 토큰을 만들고 대비비를 테스트로 검사했다.'
tags = ["접근성", "설계", "테스트"]
+++
